...
<!-- 		<label>总时间<text class="subRed">*必填</text></label>
		...
		<input placeholder="许落汀" v-model="cc" />
		
		<label>一级审批人<text class="subRed">*必填</text></label>
		<input placeholder="宋益国" v-model="oneCheck" />
		...
		<view class="pickerView">
			<picker mode="date" @change="bindOneCheckDate" style="width: 42%;position: absolute;left: 0px;" :value="'2021-' + oneCheckDate">
				<view style="line-height: 20px;color: gray; z-index: -1;" v-if="oneCheckDate == ''">审批日期</view>
				<view style="line-height: 20px;" v-if="oneCheckDate != ''">{{oneCheckDate}}</view>
			</picker>
			<picker mode="time" @change="bindOneCheckTime"style="width: 42%;position: absolute; right:0px;" :value="oneCheckTime">
				<view style="line-height: 20px;color: gray; z-index: -1;" v-if="oneCheckTime == ''">审批时间</view>
				<view style="line-height: 20px;" v-if="oneCheckTime != ''">{{oneCheckTime}}</view>
			</picker>
		</view>
		...
		methods: {
			...
			sub: function() {
				...
				if (this.name == '' || this.type == '' || this.startDate == '' || this.startTime == '' ||
					...
					this.oneCheck == '' || this.oneCheckDate == '' || this.oneCheckIdea == '') {
